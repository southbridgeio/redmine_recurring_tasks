  ...
  class IssuePresenter < SimpleDelegator
    ...
    def schedule
      ...
      if recurring_task_root.run_type == RecurringTask::RUN_TYPE_M_DAYS
        return "#{recurring_task_root.month_days_parsed.join(', ')} #{recurring_task_root.months.map { |m| I18n.t('date.month_names')[m.to_i] }.join(', ')} — #{recurring_task_root.time.strftime('%H:%M')}"
      end

      days = recurring_task_root.days.map do |field|
        <<-HTML
          <li>
            #{RecurringTask.human_attribute_name(field)}, #{recurring_task_root.time.strftime('%H:%M')}
          </li>
        HTML
      end
